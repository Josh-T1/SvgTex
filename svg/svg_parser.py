...
class SvgBuilder:
    ...
    def build_scene_items(self):
        ...
        for g_element in outer_g_elements:
            element_transform = g_element.attrib.get("transform", None)
            if element_transform is not None:
                element_transform = build_transform(element_transform)

            g_items = self.parse_element(g_element, dict(g_element.attrib), parent_transform=element_transform)
